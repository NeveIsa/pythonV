...
#sockaddr=("localhost",1234)
sockaddr=(sys.argv[2],1234)
...
while True:
  ...
  frame = cv2.addWeighted(frame,0.6,texture,0.4,0)
  ...
  for i,(new,old) in enumerate(zip(p1_new,p0_old)): # this syntax allows to take the elements of good_new  in variable new and old
    ...
    cv2.rectangle(rect_mask, (int(a-15),int(b-15)), (int(a+15),int(b+15)), (255,0,0), 3)

  img = cv2.add(frame,mask)
  img = cv2.add(img,rect_mask)

  cv2.imshow('frame',img)
  cv2.imshow("bgsub",ydiff)
...
### MODIFIED FOR DEMO
  ...
  if len(DistFeedsLA):
    ...
    #extracted4socket=[DistFeedsLA[-1],DistFeedsRA[-1],DistFeedsLB[-1],DistFeedsRB[-1],DistFeedsLC[-1],DistFeedsRC[-1],DistFeedsLD[-1],DistFeedsRD[-1],max(unVcorrAB),max(unVcorrCD)]
    extracted4socket=[DispFeedsLA[-1][0],DispFeedsRA[-1][0],DispFeedsLB[-1][0],DispFeedsRB[-1][0],DispFeedsLC[-1][0],DispFeedsRC[-1][0],DispFeedsLD[-1][0],DispFeedsRD[-1][0],max(unVcorrAB),max(unVcorrCD)]
    #extracted4socket=[DispFeedsLA[-1][1],DispFeedsRA[-1][1],DispFeedsLB[-1][1],DispFeedsRB[-1][1],DispFeedsLC[-1][1],DispFeedsRC[-1][1],DispFeedsLD[-1][1],DispFeedsRD[-1][1],max(unVcorrAB),max(unVcorrCD)]

    extracted4socket=map(int,extracted4socket)
    print extracted4socket
    #raw_input()
    sockdata=json.dumps(extracted4socket)
    print sockdata
    sock.sendto(sockdata,sockaddr)
